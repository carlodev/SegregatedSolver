...
qdegree   = 2*(order+1)
  ...
  V, Q, U, P, Y, X, model = CreateTGSpaces(mh, params, pa) #We update model with the new label of the center point
...
rhs(v) = 0.0
smatrices, A, b = compute_hierarchy_matrices(U(0.0),Auu,rhs,qdegree)
smoothers = Fill(SymGaussSeidelSmoother(5),num_levels-1)
restrictions, prolongations = setup_transfer_operators(U(0.0),qdegree;mode=:residual)
...
gmg = GMGLinearSolver(mh,
                      ...
                      verbose=false,
                      mode=:preconditioner)
ss = symbolic_setup(gmg,Mat_ML)
ns1 = numerical_setup(ss,Mat_ML)
  ...
  for tn in time_step
    ...
    while err > tol
        ...
        @time begin
          ...
          P_rich_sm  = RichardsonSmoother(JacobiLinearSolver(),10,2.0/3.0)
...
#mpiexecjl --project=. -n 4 julia Taylor_Green_Segregated_parallel_Iterative_Solvers_gamg.jl
