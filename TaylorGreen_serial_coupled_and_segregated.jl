...
include("../../Commons/Segregated/Segregated_Matrices.jl")
...
Mat_Tuu1, Mat_Tpu1, Mat_Auu1, Mat_Aup1, Mat_Apu1, Mat_App1, Mat_ML = define_matrices(0.0, V, Q, P, U)
...
norm(get_vector(AffineFEOperator(Aup,rhs,P(0.5),V)))
norm(get_vector(AffineFEOperator(App,rhs,P(0.5),Q)))
...
as = assembly_spaces(Q,V,P(0.0),U(0.0))


rhs(v) = 0.0
@time Mat_Auu1 = get_matrix(AffineFEOperator(Auu, rhs, U(0.0), V))
@time Vec_Auu1 = get_vector(AffineFEOperator(Auu, rhs, U(0.0), V))



@time Mat_Auu,Vec_Auu = Gridap.FESpaces.assemble_matrix_and_vector(Auu, rhs,as.U0, as.V)

@time Gridap.FESpaces.assemble_matrix_and_vector!(Auu,rhs, Mat_Auu,Vec_Auu, U(0.0), V)

typeof(U) <: FESpace
typeof(P) <: TransientTrialFESpace
